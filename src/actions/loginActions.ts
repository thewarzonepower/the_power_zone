...
export const login = (body: any) => async (dispatch: Function) => {
  ...
  try {
    ...
    if (status === 200 || status === 201) {
      ...
      const body2={
        applicationName:"digitalPartner"
      }
      await Auth.loginLogs(body2,data.access_token)
      const decoded = jwtDecode(data.access_token) as { username: string,userType:string,email:string,agentCode:string};
      const token = data.access_token;
      // const passwordStatus=data.user.password_change
      SecureStorage.setItem('token', token);
      SecureStorage.setItem('username', decoded?.username);
      SecureStorage.setItem('userType', decoded?.userType);
      SecureStorage.setItem('emailid', decoded?.email);
      SecureStorage.setItem('agentCode', decoded.agentCode);
...
export const changePassword = (body: any) => async (dispatch: Function) => {
  dispatch({ type: ACTIONS.SET_LOADING, payload: true });
  try {
      const { data } = await AXIOS.post(`${Prefix.api}/auth/forgot-password/`, body, {
...
export const forgotPassword = (body: any) => async (dispatch: Function) => {
  dispatch({ type: ACTIONS.SET_LOADING, payload: true });
  try {
    const { data } = await AXIOS.post(`${Prefix.api}/auth/forgot-password`,
      body,
      { headers: { Authorization: `Token ${SecureStorage.getItem("token")}` } }
    );
    dispatch({ type: ACTIONS.FORGOT_PASSWORD, payload: data });
  } catch (err: any) {
    let title = "";
    if (err.response) {
